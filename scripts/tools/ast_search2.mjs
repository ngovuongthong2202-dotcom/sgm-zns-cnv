...
function findStateUpdatesInRender(sourceFile) {
    ...
    function isInsideEffectOrCallback(node) {
        ...
        while (current) {
            if (ts.isCallExpression(current)) {
                ...
                if (['useEffect', 'useLayoutEffect', 'useCallback', 'setTimeout', 'setInterval', 'Promise', 'then', 'catch', 'finally', 'useSWR'].some(hook => text.includes(hook))) {
                    return true;
                }
            }
    ...
    function visit(node) {
        if (ts.isCallExpression(node)) {
            ...
            if (/^(set[A-Z]|dispatch)/.test(tempText)) {
