...
export function usePaymentKpiMetrics(payments: Payment[]) {
  return useMemo(() => {
    ...
    const statsByType = {
      ...
      'KHÁC': { count: 0, customers: new Set<string>() }
    };
    ...
    payments.forEach(p => {
      ...
      const refCode = `${p.soHopDong} ${p.soDonHang} ${(p as Payment & { sourceValue?: string }).sourceValue} ${p.tinhTrangThanhToan}`.toUpperCase();
      let typeStr = 'KHÁC';
    ...
    return { 
      ...
      statsByType: {
        'MÁY': { count: statsByType['MÁY'].count, customers: statsByType['MÁY'].customers.size },
        'VẬT TƯ': { count: statsByType['VẬT TƯ'].count, customers: statsByType['VẬT TƯ'].customers.size },
        'DỊCH VỤ': { count: statsByType['DỊCH VỤ'].count, customers: statsByType['DỊCH VỤ'].customers.size },
        'KHÁC': { count: statsByType['KHÁC'].count, customers: statsByType['KHÁC'].customers.size },
      }
    };
  }, [payments]);
}
