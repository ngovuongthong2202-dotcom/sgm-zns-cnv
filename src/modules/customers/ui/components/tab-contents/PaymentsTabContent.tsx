...
export function PaymentsTabContent({ loading, payments }: PaymentsTabContentProps) {
  ...
  if (loading) {
    return (
      <div className="p-8 text-center text-xs text-slate-500 animate-pulse bg-white rounded-xl border">
        Đang biên soạn lịch sử thanh toán...
      </div>
    );
  }
  ...
  if (payments.length === 0) {
    return (
      <div className="p-8 text-center text-xs bg-slate-50/50 text-slate-550 rounded-xl border border-dashed border-slate-200">
        Chưa phát sinh giao dịch thanh toán hoặc chứng từ công nợ nào.
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      {payments.map((p) => {
        const znsMeta = getStatusBadgeMeta((p as any).trangThaiGuiTinThanhToan || (p as any).trangThaiGuiTin);
        const ZnsIcon = znsMeta.icon;

        return (
          <div 
            key={p.id} 
            onClick={() => openDrawer('payment', p.id as string)}
          ...
          >
            <div className="bg-slate-50/80 px-4 py-3 border-b border-slate-100 flex flex-wrap items-center justify-between gap-2 select-none group-hover:bg-slate-50 transition-colors">
              <div className="flex items-center gap-2">
                ...
                <span className="font-mono text-xs font-bold text-slate-805 group-hover:text-blue-600 transition-colors">{getEntityDisplayLabel('payment', p)}</span>
                {p.soHopDong && (
                  <span className="font-mono bg-blue-50/60 border border-blue-100 text-blue-700 px-1.5 py-0.2 rounded text-2xs font-bold">
                    HĐ: {p.soHopDong}
                  </span>
                )}
              </div>
              <div className="flex items-center gap-1.5">
                ...
                <span className="inline-flex items-center h-5 px-1.5 rounded text-2xs font-bold bg-emerald-50 text-emerald-850 border border-emerald-100">
            ...
            <div className="p-4 space-y-3">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs text-slate-600 bg-slate-50 p-2.5 rounded-lg border border-slate-100">
                <div>
                  Ngày thu thực tế: <span className="font-semibold text-slate-800">{p.ngayThanhToan ? formatDate(p.ngayThanhToan) : '—'}</span>
                </div>
                <div>
                  Phương thức chi trả: <span className="font-medium text-slate-800">{p.phuongThucThanhToan || 'Tiền mặt/Chuyển khoản'}</span>
                </div>
              </div>
                  ...
                  <table className="w-full text-left border-collapse text-xs">
                    <thead>
                      <tr className="bg-slate-50 text-slate-550 font-semibold text-2xs uppercase tracking-wide border-b border-slate-100">
                        <th className="p-2 pl-3">Sản phẩm chi trả</th>
                        <th className="p-2 text-center w-24">Số lượng</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100">
                      {p.products.map((prod: ProductItem, idx: number) => (
                        <tr key={idx} className="hover:bg-slate-50/45 text-slate-650">
                          ...
                          <td className="p-2 text-center text-slate-500 font-mono text-2xs">{prod.quantity} {prod.unit || 'cái'}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
