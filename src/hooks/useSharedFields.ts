...
export const SWR_SYSTEM_RESOURCES_KEY = 'system_resources_15m';
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (Array.isArray(data)) {
        tinhThanhList = data.map((p: Record<string, unknown>) => p.name as string).sort();
      }
    }
  } catch (error) {
